...
public class WindosUtils {
     ...
     * 关闭无用端口
     */
    public static void closeUselessPort(){
        try {
            String [] port = cmd("netstat -ano");
            String [] timeWaitPort = new String[0];
            for(String s : port){
                if(s.contains("TIME_WAIT")){
                    timeWaitPort = Arrays.copyOf(timeWaitPort,timeWaitPort.length+1);
                    timeWaitPort[timeWaitPort.length-1] =s.substring(
                            s.indexOf(":")+1,s.indexOf(" ",s.indexOf(":")));
                }
            }
            System.out.println(Arrays.toString(timeWaitPort));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
